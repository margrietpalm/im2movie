...
def parseArgs():
    ...
    description = "im2movie creates a small but decent movie from a set of images ussing mencoder. Note that the images are added to the movie in the order they appear in the directory, make sure they are ordered!"
    parser = OptionParser(usage=usage,description=description)
    parser.add_option("-q","--quiet",action="store_true",dest="quiet",help="No output (mencoder output is always suppressed) [%default]",default=False)    
    parser.add_option("-t","--type",type="string",dest="imtype",default="png",help="type of the images (currently supports png and jpg) [%default]")
    parser.add_option("-i","--inputpath",type="string",dest="inputpath",default=os.getcwd(),help="folder with images [%default]")
    parser.add_option("-o","--outputpath",type="string",dest="outputpath",default=os.getcwd(),help="output folder [%default]")
    parser.add_option("-p","--id",type="string",dest="id",default="",help="first part of the filenames (should be unique to the sequence that is to be converted)")
    parser.add_option("--postfix",type="string",dest="postfix",default="",help="part of the filename that follow the image number (only needed whed 'id' does not suffice to select the right images")
    parser.add_option("-m","--moviename",type="string",dest="moviename",help="name of the video file (without extension) [id]")
    parser.add_option("-f","--fps",type="float",dest="fps",default=1,help="frames per second [%default]")
    parser.add_option("-x","--nx",type="int",dest="nx",help="width, used to calculate the bitrate (nx obtained from one of the images when omitted here)")
    parser.add_option("-y","--ny",type="int",dest="ny",help="heigth, used to calculate the bitrate (ny obtained from one of the images when omitted here)")
    parser.add_option("-b","--bitrate",type="int",dest="bitrate",help="user specified bitrate, when omitted the bitrate is calculated based on the image dimenstion")
    parser.add_option("-s","--scale",type="float",dest="scale",default=1,help="scale movie (useful for large input images)")
    parser.add_option("-w","--windows",action="store_true",dest="win",help="if you really really really want to use windows, this may help (or not, i wouldn't know)")
    parser.add_option("-v","--vqscale",type="int",dest="vqscale",default=12,help="quality, lower is better (but file size is larger) [%default]")
...
def makeMovie(id,imtype,moviename,inputpath,outputpath,fps,nx=None,ny=None,bitrate=None,scale=1,quiet=True,win=False,
              vqscale=12,suffix='.avi',tomp4=False,postfix=None, maxres=(2000,2000)):
...
#        print 'avconv -i '+outputpath+moviename+suffix+' -c:v libx264 '+outputpath+moviename+'.mp4'
        os.system('avconv -i '+outputpath+moviename+suffix+' -y -c:v libx264 '+outputpath+moviename+'.mp4')
...
def main():
    ...
    if not opt.outputpath.endswith('/'):
        opt.outputpath += '/'        
    makeMovie(opt.id,opt.imtype,opt.moviename,opt.inputpath,opt.outputpath,opt.fps,nx=opt.nx,ny=opt.ny,bitrate=opt.bitrate,scale=opt.scale,quiet=opt.quiet,win=opt.win,vqscale=opt.vqscale,tomp4=opt.mp4,postfix=opt.postfix)
